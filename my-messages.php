...
$messages = DB::query('SELECT messages.*, users.username FROM messages, users WHERE receiver=:receiver OR sender=:sender AND users.id = 
messages.sender', array(':receiver'=>$userid, ':sender'=>$userid));
foreach ($messages as $message) {
	...
	if ($message['read'] == 0) {
			...
			echo "<a href='my-messages.php?mid=".$message['id']."'>".$message['body']."</a> sent by ".$message['username']."<hr />";
		}
	}
}
